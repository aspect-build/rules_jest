...
module "remote_cache_dns" {
  ...
  zones = {
    "rules-jest-remote-cache.aspect.build" = {
      domain_name = "rules-jest-remote-cache.aspect.build"
      comment     = "rules-jest-remote-cache.aspect.build"
      tags = {
        Name = "rules-jest-remote-cache.aspect.build"
      }
    }
  }
}
...
module "aspect_workflows" {
  ...
  external_remote_cache = {
    public_hosted_zone_id = module.remote_cache_dns.route53_zone_zone_id["rules-jest-remote-cache.aspect.build"]
    cache_shards          = 1
    frontend = {
      max_scaling = 5
      min_scaling = 1

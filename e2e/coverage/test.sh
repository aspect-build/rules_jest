...
assert_covered() {
	...
	# DA records are scoped to the preceding SF record. Do not accept an executed
	# line from an unrelated source in the same report.
	if ! awk -v source="SF:$src" '
		$0 == source { in_source = 1; next }
		in_source && /^DA:[0-9]+,[1-9][0-9]*$/ { covered = 1 }
		in_source && /^end_of_record$/ { exit(covered ? 0 : 1) }
